 ...
 */
OpenLayers.Editor.GPX = OpenLayers.Class(OpenLayers.Format.XML, {
  ...
  read: function(doc) {
    ...
    if (this.extractWaypoints) {
      ...
      for (var l = 0, len = waypoints.length; l < len; l++) {
        var attrs = this.parseAttributes(waypoints[l]);
        var wpt = new OpenLayers.Geometry.Point(waypoints[l].getAttribute("lon"), waypoints[l].getAttribute("lat"), attrs.ele);
  ...
  extractSegment: function(segment, segmentType) {
    ...
    for (var i = 0, len = points.length; i < len; i++) {
      var attrs = this.parseAttributes(points[i]);

      point_features.push(new OpenLayers.Geometry.Point(points[i].getAttribute("lon"), points[i].getAttribute("lat"), attrs.ele));
    }
  ...
  buildMetadataNode: function(metadata) {
    var types = ['name', 'desc', 'author'],
      node = this.createElementNSPlus('metadata');
  ...
  buildTrkPtNode: function(point) {
    ...
    if (point.z !== null) {
      var n = this.createElementNSPlus("ele");
      n.appendChild(this.createTextNode(point.z));
      node.appendChild(n);
    }
    return node;
  },
